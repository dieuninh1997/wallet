  ...
  numeral._ = _ = {
    ...
    toFixed(value, maxDecimals, roundingFunction, optionals) {
      ...
      if (splitValue.length === 2) {
        ...
        const index = value.toString().indexOf('e');
        if (index >= 0) {
          const e = value.toString().substring(index + 1);
          if (e < 0) {
            boundedPrecision = -e;
          }
        } else {
      ...
      const valueToRound = value.toString().indexOf('e') >= 0 ? (value * power) : (`${value}e+${boundedPrecision}`);
